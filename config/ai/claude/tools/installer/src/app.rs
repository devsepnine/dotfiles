...
impl App {
    ...
    pub fn process_step(&mut self) -> Result<bool> {
        if self.processing_queue.is_empty() {
            return Ok(true); // Done
        }

        let idx = self.processing_queue.remove(0);
        let progress = self.processing_progress.unwrap_or(0) + 1;
        self.processing_progress = Some(progress);

        if self.is_removing {
            self.process_remove_step(idx)?;
        } else {
            self.process_install_step(idx)?;
        }

        Ok(self.processing_queue.is_empty())
    }

    fn process_install_step(&mut self, idx: usize) -> Result<()> {
        if self.tab == Tab::McpServers {
            if let Some(server) = self.mcp_servers.get_mut(idx) {
                let name = server.def.name.clone();
                let scope_info = if self.mcp_scope == McpScope::Local {
                    format!("local: {}", self.mcp_project_path)
                } else {
                    "user".to_string()
                };
                self.processing_log.push(format!("Installing {} (scope: {})...", name, scope_info));

                // Get env values if this is the server we collected env vars for
                let env_values: Vec<(String, String)> = if self.env_input_server_idx == Some(idx) {
                    self.env_input_values.clone()
                } else {
                    Vec::new()
                };

                // Get project path for local scope
                let project_path = if self.mcp_scope == McpScope::Local {
                    Some(self.mcp_project_path.as_str())
                } else {
                    None
                };

                match fs::installer::install_mcp_server(&server, self.mcp_scope, project_path, &env_values) {
                    Ok(warning) => {
                        self.processing_log.push(format!("[OK] {}", name));
                        if !warning.is_empty() {
                            self.processing_log.push(format!("  {}", warning));
                        }
                        // Update status immediately on success
                        server.status = crate::mcp::McpStatus::Installed;
                    }
                    Err(e) => {
                        self.processing_log.push(format!("[ERR] {}: {}", name, e));
                    }
                }
            }
        } else if self.tab == Tab::Plugins {
            if let Some(plugin) = self.plugins.get_mut(idx) {
                ...
                self.processing_log.push(format!("Installing plugin {}...", name));

                match fs::installer::install_plugin(&plugin) {
                    Ok(output) => {
                        self.processing_log.push(format!("[OK] {}", name));
                        if !output.is_empty() {
                            self.processing_log.push(format!("  {}", output));
                        }
                        // Update status immediately on success
                        plugin.status = crate::plugin::PluginStatus::Installed;
                    }
                    Err(e) => {
                        self.processing_log.push(format!("[ERR] {}: {}", name, e));
                    }
        ...
        } else if let Some(component) = self.components.get(idx) {
            let name = component.name.clone();
            self.processing_log.push(format!("Installing {}...", name));

            match fs::installer::install_component(component, &self.source_dir, &self.dest_dir) {
                Ok(_) => {
                    self.processing_log.push(format!("[OK] {}", name));
                }
                Err(e) => {
    ...
    fn process_remove_step(&mut self, idx: usize) -> Result<()> {
        if self.tab == Tab::McpServers {
            if let Some(server) = self.mcp_servers.get_mut(idx) {
                let name = server.def.name.clone();
                self.processing_log.push(format!("Removing {}...", name));

                match fs::installer::remove_mcp_server(&server) {
                    Ok(_) => {
                        self.processing_log.push(format!("[OK] {}", name));
                        // Update status immediately on success
                        server.status = crate::mcp::McpStatus::NotInstalled;
                    }
                    Err(e) => {
                        self.processing_log.push(format!("[ERR] {}: {}", name, e));
                    }
            ...
            if let Some(plugin) = self.plugins.get_mut(idx) {
                let name = plugin.def.name.clone();
                self.processing_log.push(format!("Removing plugin {}...", name));

                match fs::installer::remove_plugin(&plugin) {
                    Ok(_) => {
                        self.processing_log.push(format!("[OK] {}", name));
                        // Update status immediately on success
                        plugin.status = crate::plugin::PluginStatus::NotInstalled;
                    }
                    Err(e) => {
                        self.processing_log.push(format!("[ERR] {}: {}", name, e));
                    }
            ...
            if component.dest_path.exists() {
                match std::fs::remove_file(&component.dest_path) {
                    Ok(_) => {
                        self.processing_log.push(format!("[OK] {}", name));
                    }
                    Err(e) => {
                ...
                self.processing_log.push(format!("[SKIP] {} (not installed)", name));
            }
        }
        Ok(())
    }
